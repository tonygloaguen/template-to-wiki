...
class HatlabOdtConverterGui(tk.Tk):
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            root,
            text="Convertisseur ODT Hatlab vers DokuWiki",
            font=("TkDefaultFont", 13, "bold"),
        )
        title.grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 12))

        ttk.Label(root, text="Fichier ODT source").grid(
            row=1, column=0, sticky="w", pady=4
        )
        ttk.Entry(root, textvariable=self.input_odt).grid(
            row=1, column=1, sticky="ew", padx=8, pady=4
        )
        ttk.Button(
            root, text="Choisir un fichier .odt", command=self.choose_input
        ).grid(row=1, column=2, sticky="ew", pady=4)

        ttk.Label(root, text="Fichier TXT de sortie").grid(
            row=2, column=0, sticky="w", pady=4
        )
        ttk.Entry(root, textvariable=self.output_txt).grid(
            row=2, column=1, sticky="ew", padx=8, pady=4
        )
        ttk.Button(
            root, text="Choisir l'emplacement de sortie", command=self.choose_output
        ).grid(row=2, column=2, sticky="ew", pady=4)

        ttk.Label(root, text="Namespace média DokuWiki").grid(
        ...
        ttk.Entry(namespace_frame, textvariable=self.media_namespace).grid(
            row=0, column=0, sticky="ew"
        )
        ttk.Label(
            namespace_frame, text="Exemples : projets:electronique, projets:maison"
        ).grid(row=1, column=0, sticky="w", pady=(2, 0))

        ttk.Label(root, text="Identifiant de page").grid(
            row=4, column=0, sticky="w", pady=4
        )
        page_frame = ttk.Frame(root)
        page_frame.grid(row=4, column=1, sticky="ew", padx=8, pady=4)
        page_frame.columnconfigure(0, weight=1)
        ttk.Entry(page_frame, textvariable=self.page_id).grid(
            row=0, column=0, sticky="ew"
        )
        ttk.Label(page_frame, text="Exemple : install_party_linux").grid(
            row=1, column=0, sticky="w", pady=(2, 0)
        )

        ttk.Checkbutton(
            root,
            text="Extraire les images intégrées",
            variable=self.extract_images,
            command=self._update_extract_state,
        ...
        self.extract_button = ttk.Button(
            root, text="Choisir le dossier média", command=self.choose_media_dir
        )
        self.extract_button.grid(row=5, column=2, sticky="ew", pady=4)

        ttk.Checkbutton(
            root, text="Écraser le fichier de sortie s'il existe", variable=self.force
        ...
        ttk.Button(button_frame, text="Convertir", command=self.convert).grid(
        ...
        log_frame.grid(
            row=8, column=1, columnspan=2, sticky="nsew", padx=8, pady=(6, 0)
        )
    ...
    def choose_output(self):
        ...
        if self.input_odt.get().strip():
            initialfile = (
                os.path.splitext(os.path.basename(self.input_odt.get().strip()))[0]
                + ".txt"
            )
        ...
        if self.extract_images.get():
            ...
            if not extract_media_dir:
                raise ConversionError(
                    "Choisissez un dossier média pour extraire les images intégrées."
                )
    ...
    def convert(self):
        try:
            input_odt, output_txt, media_namespace, page_id, extract_media_dir = (
                self._validate_form()
            )
            self.log("--- Conversion ---")
            ...
            messagebox.showinfo(
                "Conversion terminée", "Fichier généré :\n%s" % report["output_txt"]
            )
    ...
    def open_output_folder(self):
        ...
        if not output_txt:
            messagebox.showwarning(
                "Aucun fichier", "Aucun fichier de sortie n'est défini."
            )
            return
        folder = os.path.dirname(os.path.abspath(output_txt))
        if not os.path.isdir(folder):
            messagebox.showerror(
                "Dossier introuvable", "Le dossier n'existe pas :\n%s" % folder
            )
            return
        try:
